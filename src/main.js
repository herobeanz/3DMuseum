...
control.setMoveSpeed(12); // Giảm từ 20 xuống 12 — di chuyển êm hơn
control.setMouseSensitivity(0.12, 0.06); // Độ nhạy chuột thấp hơn
control.setRotationSmoothFactor(6); // Làm mượt xoay camera
control.setVerticalLookLimit(65); // Giới hạn ngước/nhìn xuống ±65°
// Giới hạn di chuyển trong phòng (sàn 200x100, trừ lề để không đụng tường)
control.setBounds(-98, 98, -48, 48);
...
// Lấy tên hiển thị: nếu click vào mesh con (vd discobolus_1) thì dùng tên object gốc (Discobolus)
function getDisplayName(obj) {
    let current = obj;
    while (current) {
        if (objects.indexOf(current) !== -1) return current.name;
...
function togglePopup(state) {
    ...
    popUp.innerHTML = getDisplayName(intersectedObject)
